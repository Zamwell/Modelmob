...
#    print(profil_mob)
    ...
    data_typjour.loc[:,'V2_MORIHDEP'] = pd.to_timedelta(data_typjour['V2_MORIHDEP'],errors='coerce')
...
#            print(nb_tranchlois)
            ...
            dic_tranchlois[typ] = pg.DiscreteDistribution(dict(zip([x for x in tranche_hor_lois],nb_tranchlois)))
...
#    print(profil_lois_domlois)
    ...
    for typ in typjour:
        if "loisirs" in typ:
            nb_parklois = [data_typjour[data_typjour['TYPE_JOUR']==typ][x].sum() for x in park_lois]
            nb_parklois = [x/sum(nb_parklois) for x in nb_parklois]
            dic_parklois[typ] = pg.DiscreteDistribution(dict(zip(['gpark','ppark'],nb_parklois)))
    ...
    df_dureelois=df[['IDENT_IND','V2_DURACT', 'V2_MMOTIFDES']].merge(data_typjour[['IDENT_IND', 'TYPE_JOUR']], left_on ='IDENT_IND', right_on = 'IDENT_IND', how='left')
    for i,typ in enumerate(typjour):
        if "loisirs" in typ:
            dic_dureelois[typ] = {}
            dur_ppark = df_dureelois[(df_dureelois['TYPE_JOUR']==typ) & (df_dureelois['V2_MMOTIFDES'].apply(est_petit_parking)) & (df_dureelois['V2_DURACT']>0) & (df_dureelois['V2_DURACT']<301)]['V2_DURACT']
            dur_gpark = df_dureelois[(df_dureelois['TYPE_JOUR']==typ) & (df_dureelois['V2_MMOTIFDES'].apply(est_grand_parking)) & (df_dureelois['V2_DURACT']>0) & (df_dureelois['V2_DURACT']<301)]['V2_DURACT']
...
def hist_lois(li):
    ...
    for tu in li:
        if tu[0] == 2 or tu[0] == 3:
            hist[pd.to_timedelta(tu[1]).seconds//(3600//2)]+=1
    return hist
